...
export function trapFocus(el: HTMLElement, toggleEl: HTMLElement, callback: () => void) {
  const focusableEls = [toggleEl, ...Array.from(el.querySelectorAll(focusableSelectors))] as HTMLElement[];
  ...
  const handleListeners = (shouldListen: boolean) => {
    if (shouldListen) {
      ...
      toggleEl.addEventListener('keydown', handleTrapFocus);
    } else {
      el.removeEventListener('keydown', handleTrapFocus);
      toggleEl.removeEventListener('keydown', handleTrapFocus);
      toggleEl.focus();
    }
  };
